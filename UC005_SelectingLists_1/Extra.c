...
Check(int status, char *tr_name)
{
	if(status == LR_FAIL){
		...
		lr_end_transaction("MAIN_SelectingList", LR_FAIL);
		Relogin();
...
Relogin()
{
	...
	lr_start_transaction("_UC005_SelectingLists_Relogin");
	...
	lr_end_transaction("_UC005_SelectingLists_Relogin", LR_FAIL);
